...
`cssgraph index` on a production monorepo (9,400+ files) initially took **9 minutes**.
...
| R2 | Unchanged files skip parse | Second `index` processes only changed files |
...
- [x] production monorepo `index` initial: **3m 36s** (P0 applied, was 5m08s before any perf work, 9m04s at baseline)
- [x] production monorepo `index` repeat (no changes): 3m 05s parsing phase
